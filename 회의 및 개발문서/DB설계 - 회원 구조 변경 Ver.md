...
| note (사유) | X | 해당 상태를 부여한 이유가 있다면 그 이유를 적음 / OPTIONAL  |
...
| infra_no (시설 번호) | 외래 키 (시설 테이블 참조)<br>/ OPTIONAL | 비어 있는 경우도 허용(학교 전체 적용 등) |
...
| infra_room_no<br>(시설 호실(강의실, 교수실 번호)) | X / (3개 속성 묶어서 UNIQUE) | 90 |
| bulid_no (건물 번호) | 외래 키 (학교 건물 테이블 참조) <br>(3개 속성 묶어서 UNIQUE) | 제주 서귀포시 토평동 산15-1 (제주 한라산 백록담) |
...
| reserv_no (시설 예약 번호) | 외래 키 (시설 예약 테이블 참조)<br>OPTIONAL | 시설 예약 코드가 비어있는 경우, 새로운 시설 예약을 신청하는 것으로 간주 |
...
| vote_no (투표 번호) | 외래 키 (투표 테이블 참조)<br><br>UNIQUE 속성 (투표 코드/일정 시작/종료/시설코드를 묶어서) |  |
...
| infra_no (시설 번호) | 외래 키 (시설 테이블 참조) + UNIQUE 속성 (투표 코드/일정 시작/종료/시설코드를 묶어서) |  |
