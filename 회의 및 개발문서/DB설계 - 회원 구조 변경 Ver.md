...
![YouniverseCity ERD- 회원 구조 변경 후](YoniverseCity-ERD-회원변경.png)


- 회원 테이블 (Member) 필요 논의 사항
    - 복수전공 / 부전공 처리 방식 (복수전공 OR 부전공을 합해 3개인 경우까지 고려)
        - 추후 확장 시 지원하기로 결정 (고려할 부분이 많고 도메인에 대한 이해가 부족)
    - 회원 정보를 저장할 때(초기 데이터 저장 시점), Member 테이블, MemberRole 테이블, Student or Professor 테이블 및 해당 테이블의 Status 테이블까지 트랜잭션으로 관리해야 한다. (해당 회원의 현재 역할, 그에 따른 전용 정보 입력, 해당 회원의 상태까지 모두 한번에 입력되어야 한다.)

| 필드명 (논리명) | 키 여부 | 비고 |
| --- | --- | --- |
| member_no (회원 번호) |  기본 키 | 자동 증가(autoincrement) |
| memberID (회원ID) | X | ex) abcdef123 <br>UNIQUE 속성 |
| password (비밀번호) | X | ex) abc123!@(암호화) |
| email (이메일) | X | ex) abcd@abc.def |
| member_name (이름) | X | ex) 홍길동 |
| phone_number (전화번호) | X | ex) 010-1234-5678 <br> 전화번호 형식 검사 |
...
- 회원 역할 테이블 (MemberRole)
    - 회원의 권한을 관리하는 회원(Member) 하위 테이블
    - 해당 테이블은 `학생, 교수, 관리자, 조교, 행정직` 등 해당 회원의 역할 및 권한을 저장한다.

| 필드명 (논리명) | 키 여부 | 비고 |
| --- | --- | --- |
| role_no (역할 번호) | 기본 키 | 자동 증가 (autoincrement) |
| member_no (회원 번호) | 외래 키 (회원 테이블 참조) | UNIQUE 속성 (회원 번호 + 역할 이름 + 적용일)  |
| role_name (역할 이름) | X | 역할 이름이 같은 기간내에 중복되는 경우, 겸직이 불가능한 역할이 추가되는 경우 (ex. 학생으로 재학중인 경우, 교수로 임용할 수 없음) <br>트리거 및 제약 조건을 통해 검사 <br>UNIQUE 속성 (회원 번호 + 역할 이름 + 적용일)  |
| from_date (적용일) | X | UNIQUE 속성 (회원 번호 + 역할 이름 + 적용일)  |
| to_date (종료일) | X | 종료일은 Nullable <br>종료일자가 없는 경우(NULL), 해당 역할을 수행중임을 의미함 <br>종료일은 적용일 이후의 날짜만 입력 가능 |
- Student, Professor 테이블 공통 (제약 조건)
    - `MemberRole` 테이블에서 `role_type`이 `STUDENT, PROFESSOR`이면서 동시에 `to_date` 가 Null이거나 현재 날짜 이후인 경우를 검사 (재학생, 재직 여부 판별)
- StudentStatus, ProfessorStatus 테이블 공통
    - 재학 - 휴학 - 정학 - 퇴학 - 졸업 OR 재직 - 휴직 - 사직 - 해고 등의 상태에 관련한 조건 지정
        - `재학 및 재직` 의 경우, `졸업, 사직, 정학, 퇴학, 해고` 의 상태가 지정되는 경우 해당 상태의 적용일이 `재학` 상태의 종료일로 입력해야 함
        또한 `회원 역할`  테이블에도 같은 회원의 모든 `NULL 상태의 종료일을 해당 일자로 갱신`해야 함
        - `휴학, 정학, 휴직` 상태가 종료되지 않은 경우, `재학 및 재직` 의 `종료일자` 를 입력할 수 없도록 해야 함 + `졸업, 사직, 해고` 또한 데이터 입력 불가
        - `졸업, 퇴학, 사직, 해고` 등의 데이터가 존재하는 경우, 나머지 상태는 해당 일자 이후의 데이터 입력 불가 + 새로운 데이터도 입력 불가

- 학생 테이블 (Student)
    - 학생만 필요한 정보를 저장할 회원(Member) 하위 테이블

| 필드명 (논리명) | 키 여부 | 비고 |
| --- | --- | --- |
| member_no (회원 번호) | 기본 키 (회원 테이블 참조) | 외래 키를 기본 키로 사용(식별 관계) |
| student_no (학번) | X | 자동 생성 키를 사용하여 저장 <br> 연도별 OR 학과별로 키를 따로 만드는 방법 고려하기 |
| department_code (학과 코드) | 외래 키 (학과 테이블 참조) | - |
- 학생 상태 테이블 (StudentStatus)
    - 학생의 현재 상태 (휴학 여부 등)을 저장할 회원(Member) 하위 테이블
    - `재학, 휴학, 정학, 퇴학, 졸업` 등 해당 학생의 현재 상태를 저장한다.

| 필드명 (논리명) | 키 여부 | 비고 |
| --- | --- | --- |
| status_no (상태 번호) | 기본 키 | 자동 증가(autoincrement) |
| member_no (회원 번호) | 외래 키 (학생 테이블 참조) | - |
| student_status (학생 상태) | X | 재학, 휴학, 정학, 퇴학, 졸업 등 <br>휴학 등은 COUNT를 통해 횟수 제한 <br>  |
| from_date (상태 적용일) | X | - |
| to_date (상태 종료일) | X | 종료일이 없거나, 종료일이 지나지 않았다면 해당 상태가 부여중임을 의미 |
| note (사유) | X | 해당 상태를 부여한 이유가 있다면 그 이유를 적음<br>OPTIONAL  |

- 교수 테이블 (Professor)
    - 교수에게만 필요한 정보를 저장할 회원(Member) 하위 테이블

| 필드명 (논리명) | 키 여부 | 비고 |
| --- | --- | --- |
| member_no (회원 번호) | 기본 키 (회원 테이블 참조) | 외래 키를 기본 키로 사용(식별 관계) |
| professor_no (교번) | X | 자동 생성 키를 사용하여 저장 <br> 연도별 OR 학과별로 키를 따로 만드는 방법 고려하기 |
| faculty_code (학부 코드) | 외래 키 (학부 테이블 참조) | - |
| department_code (학과 코드) | 외래 키 (학과 테이블 참조) | - |
- 교수 상태 테이블 (ProfessorStatus)

| 필드명 (논리명) | 키 여부 | 비고 |
| --- | --- | --- |
| status_no (상태 번호) | 기본 키  | 자동 증가(autoincrement) |
| member_no (회원 번호) | 외래 키 (교수 테이블 참조) | - |
| professor_status (교수 상태) | X | 휴직, 재직, 사직 등 |
| from_date (상태 적용일) | X | - |
| to_date (상태 종료일) | X | 종료일이 없거나, 종료일이 지나지 않았다면 해당 상태가 부여중임을 의미 |
| note (사유) | X | 해당 상태를 부여한 이유가 있다면 그 이유를 적음<br>OPTIONAL |
...
| 필드명 (논리명) | 키 여부 | 비고 |
| --- | --- | --- |
| user_schedule_no<br>(개인 일정 번호) | 기본 키 | 자동 생성 (autoincrement) |
| member_no (회원 번호) | 외래 키 (회원 테이블 참조) | - |
| schedule_name (일정 이름) | X | - |
| from_date (일정 시작 시간) | X | YY-MM-DD HH:MM |
| to_date (일정 종료 시간) | X | YY-MM-DD HH:MM<br>일정 종료 시간은 일정 시작 시간 이후의 시간만 입력 가능 |
- 학부 테이블 (Faculty)
...
| 필드명 (논리명) | 키 여부 | 비고 |
| --- | --- | --- |
| requirement_no (조건 번호) | 기본 키 | 자동 생성 |
- 학부 수강 조건 테이블 (FacultyRequirements)

| 필드명 (논리명) | 키 여부 | 비고 |
| --- | --- | --- |
| requirement_no (조건 번호) | 복합 키 (수강 조건 테이블 참조) | ex) A |
...
| 필드명 (논리명) | 키 여부 | 비고 |
| --- | --- | --- |
| CS_no (강의 일정 번호) | 기본 키 | 자동 생성(autoincrement) |
- 강의 시간표 테이블 (CourseClassSchedule)

| 필드명 (논리명) | 키 여부 | 비고 |
| --- | --- | --- |
| schedule_day (요일) | 복합 키 / 요일 | - |
| CS_no (강의 일정 번호) | 복합 키 (강의 일정 테이블 참조) | - |
...
| time_from (시작 시간) | X | HH:MM<br><br>시작 시간은 종료 시간 이후가 될 수 없다. |
| time_to(종료 시간) | X | HH:MM<br><br>종료 시간은 시작 시간 이전이 될 수 없다. |
- 강의 테이블 (Course)
...
| course_from (강의 시작 날짜) | X | ex) 26.03.02 : 년도가 강의 년도랑 일치하는지를 검사하는 제약 조건 설정<br><br>강의 시작 날짜는 현재 날짜 이후로만 입력 가능 |
| course_to (강의 종료 날짜) | X | ex) 26.07.01 : 강의 종료일은 강의 시작 날짜 이후로만 입력 가능하도록 제약 조건 설정<br><br>강의 종료 날짜는 강의 시작 날짜 이후로만 입력 가능 |
| course_time (강의 시간) | 외래 키 (강의 일정 테이블 참조) | 화78 목78<br>OPTIONAL |
...
| classroom (강의실) | 외래 키 (시설 테이블 참조)  | ex) 18110<br>OPTIONAL |
| course_explain (강의 설명) | X | 공학 계열만 수강 가능, 단 컴퓨터공학과는 수강불가 |
| member_no (담당교수번호) | 외래 키 (교수 테이블 참조) | - |
- 수강 테이블 (Enrollments)
    - 순수 수강 정보만 담는 테이블
    - 재수강 조건 관련
...
| 필드명 (논리명) | 키 여부 | 비고 |
| --- | --- | --- |
| member_no (회원 번호) | 복합 키(학생 테이블 참조) | - |
...
| idle_from (시작 시간) | X | YYYY-MM-DD HH:MM<br><br>현재 시간 이후의 데이터만 입력 가능 |
| idle_to (종료 시간) | X | YYYY-MM-DD HH:MM<br><br>종료 시간은 시작 시간 이후의 데이터만 입력 가능 |
...
| 필드명 (논리명) | 키 여부 | 비고 |
| --- | --- | --- |
| common_schedule_no<br>(공유 일정 번호) | 기본 키 | 자동 생성 |
| schedule_class (일정 분류) | 외래 키 (일정 분류 테이블 참조) | - |
| time_from (시작 시간) | X | YY-MM-DD HH:MM<br><br>시작 시간은 현재 시간 이후의 데이터만 입력 가능 |
| time_to (종료 시간) | X | YY-MM-DD HH:MM<br><br>종료 시간은 시작 시간 이후의 데이터만 입력 가능 |
...
| infra_no (시설 번호) | 외래 키 (시설 테이블 참조) | 비어 있는 경우도 허용(학교 전체 적용 등)<br>OPTIONAL |
...
| 필드명 (논리명) | 키 여부 | 비고 |
| --- | --- | --- |
| infra_no (시설 번호) | 기본 키 | 자동 생성 |
| infra_name (시설명) | X | 18490 강의실 |
| infra_class (시설 분류) | 외래 키 (시설 분류 테이블 참조) | 강의실 / 도서실 / 교수실 / 회의실 등 |
| infra_max_count<br>(시설 수용 인원(정원)) | X | 100 |
| infra_room_no<br>(시설 호실(강의실, 교수실 번호)) | X | 90<br>UNIQUE 속성 (호실 + 건물 번호 + 층수) |
| bulid_no (건물 번호) | 외래 키 (학교 건물 테이블 참조) <br> | 제주 서귀포시 토평동 산15-1 (제주 한라산 백록담)<br>UNIQUE 속성 (호실 + 건물 번호 + 층수) |
| infra_floor (층수) | X | 1층 <br>UNIQUE 속성 (호실 + 건물 번호 + 층수) |
...
| 필드명 (논리명) | 키 여부 | 비고 |
| --- | --- | --- |
| reserv_no (시설 예약 번호) | 기본 키 | 자동생성 |
| course_code (강의 코드) | 외래 키 (강의 테이블 참조) | OPTIONAL (강의코드 + 강의 분반 + 강의 년도) |
| course_class (강의 분반) | 외래 키 (강의 테이블 참조) | OPTIONAL (강의코드 + 강의 분반 + 강의 년도) |
| course_year (강의 년도) | 외래 키 (강의 테이블 참조) | 제약 조건은 현재 년도만 가능하도록 설정<br><br>→ 현재 년도가 바뀌었을 시 (예 : 2026년도에서 2027년도가 되었을때)<br><br>제약 조건이 정상적으로 동작하는지 확인 필요 (기존 데이터는 문제없는지, 2027년도 데이터가 성공적으로 입력되는지 확인)<br>OPTIONAL (강의코드 + 강의 분반 + 강의 년도) |
| infra_no (시설 번호) | 외래 키 (시설 테이블 참조) | 5110 |
| time_from (예약 시작 시간) | X | YYYY-MM-DD HH:MM<br><br>예약 시작 시간은 현재 시간보다 더 이후여야 함 |
| time_to (예약 종료 시간) | X | YYYY-MM-DD HH:MM<br><br>예약 종료 시간은 예약 시작 시간보다 더 이후여야 함 |
...
| 필드명 (논리명) | 키 여부 | 비고 |
| --- | --- | --- |
| vote_no (투표 번호) | 기본 키 | 자동 생성 |
| course_code (강의 코드) | 외래 키 (강의 테이블 참조) | - |
| course_class (강의 분반) | 외래 키 (강의 테이블 참조) | - |
| course_year (강의 년도) | 외래 키 (강의 테이블 참조) | 제약 조건은 현재 년도만 가능하도록 설정<br><br>→ 현재 년도가 바뀌었을 시 (예 : 2026년도에서 2027년도가 되었을때)<br><br>제약 조건이 정상적으로 동작하는지 확인 필요 (기존 데이터는 문제없는지, 2027년도 데이터가 성공적으로 입력되는지 확인) |
| vote_name (투표명) | X | ex) 중간고사, 기말고사, 보강 등 |
| vote_status (투표 상태) | X | 투표 중 / 투표 완료 / 투표 마감 등 |
| vote_time_from (투표 시작 시간) | X | YYYY-MM-DD HH:MM<br><br>시작 시간은 현재 시간보다 더 이후여야 함 |
| vote_time_to (투표 마감 시간) | X | YYYY-MM-DD HH:MM<br><br>시작 시간은 현재 시간보다 더 이후여야 함 |
| reserv_no (시설 예약 번호) | 외래 키 (시설 예약 테이블 참조) | 시설 예약 코드가 비어있는 경우, 새로운 시설 예약을 신청하는 것으로 간주<br>OPTIONAL |
- 투표 선택지 테이블 (VoteSelection)

| 필드명 (논리명) | 키 여부 | 비고 |
| --- | --- | --- |
| selection_no (투표 선택지 번호) | 기본 키 | 자동 생성 |
| selection_name (투표 선택지 이름) | X | ex) 목요일 저녁 |
| vote_no (투표 번호) | 외래 키 (투표 테이블 참조) | UNIQUE 속성 (투표 번호+일정 시작 시간+종료 시간+시설번호) |
| selection_start_time<br>(일정 시작 시간) | X | YYYY-MM-DD HH:MM<br><br>시작 시간은 현재 시간보다 더 이후여야 함<br>UNIQUE 속성 (투표 번호+일정 시작 시간+종료 시간+시설번호) |
| selction_end_time (일정 종료 시간) | X | YYYY-MM-DD HH:MM<br><br>시작 시간은 현재 시간보다 더 이후여야 함<br>UNIQUE 속성 (투표 번호+일정 시작 시간+종료 시간+시설번호) |
| infra_no (시설 번호) | 외래 키 (시설 테이블 참조)) | UNIQUE 속성 (투표 번호+일정 시작 시간+종료 시간+시설번호) |
...
| 필드명 (논리명) | 키 여부 | 비고 |
| --- | --- | --- |
| member_no (회원 번호) | 복합 키 (회원 테이블 참조) | 투표 권한 체크 |
| selection_no (투표 선택지 번호) | 복합 키 (투표 선택지 테이블 참조) | 해당 테이블에 입력된 투표선택지 코드는 해당 학생이 선택지에 ‘Y’ 표시를 한 것으로 간주 |
